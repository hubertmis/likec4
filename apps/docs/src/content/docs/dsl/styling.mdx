...
- Style all elements matching an expression (tags, fully qualified names, wildcards) in `specification`
...
#### Elements matching an expression

To style all elements matching an expression, use the `style` block in `specification`:
...
specification {
  ...
  style element.tag = #deprecated {
    // every element with tag 'deprecated'
    color muted
  }

  style element.tag != #deprecated {
    // every element without tag 'deprecated'
    opacity 20%
  }
...
<Aside type='tip'>
  To reduce boilerplate, `icon` may be defined as a property of the element.
